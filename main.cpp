...
void setup_catppuccin_mocha_theme()
{
    ...
    colors[ImGuiCol_TitleBg] = ImVec4(0.109f, 0.109f, 0.156f, 1.0f);
...
void shootBullet()
{
    ...
    btRigidBody* bullet = addDynamicBox(btVector3(bulletSize, bulletSize, bulletSize), glmToBt(startPos), bulletMass
    );
...
public:
    ...
    {
        springs.push_back({ a, b, restLength });
    }
...
void renderScene(Shader& shader)
{
    ...
    for (btRigidBody* rb : rigidBodies)
    {
        ...
        if (shape->getShapeType() == BOX_SHAPE_PROXYTYPE)
        {
            ...
            btVector3 he = bx->getHalfExtentsWithoutMargin();
        ...
        {
            shader.setVec3("objectColor", glm::vec3(0.8f, 0.8f, 0.2f));
        }
        cubeIndex++;
...
int main()
{
    ...
    addDynamicBox(btVector3(1.0f, 1.0f, 1.0f), btVector3(-4.0f, 1.0f, 0.0f), 5.0f);
    addDynamicBox(btVector3(0.8f, 0.8f, 0.8f), btVector3(4.0f, 0.8f, 0.0f), 3.0f);

    btCollisionShape* playerShape = new btCapsuleShape(playerRadius, playerHeight - 2.0f * playerRadius);
    collisionShapes.push_back(playerShape);

    btTransform playerTransform;
    playerTransform.setIdentity();
    playerTransform.setOrigin(btVector3(0.0f, 1.6f, 0.0f));

    btVector3 playerInertia(0, 0, 0);
    playerShape->calculateLocalInertia(80.0f, playerInertia);

    btDefaultMotionState* playerMotion = new btDefaultMotionState(playerTransform);
    btRigidBody::btRigidBodyConstructionInfo playerInfo(80.0f, playerMotion, playerShape, playerInertia);
    ...
    // проверка на наличие шейдеров
    if (!lightingShader->ID || !simpleShader->ID || !skyboxShader->ID || !depthShader->ID) {
        std::cerr << "Failed to load shaders!" << std::endl;
        std::cerr << "Make sure shaders/ directory exists with .vert and .frag files" << std::endl;
        cleanup();
        glfwTerminate();
        return -1;
    }
    ...
    std::cout << "Flux3D Engine started successfully!" << std::endl;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (cameraMode && playerBody)
        {
            ...
            if (!spaceDown) spacePressed = false;
        }
        ...
        if (shadowsEnabled)
        {
            ...
            // создание динамических обьектов и коллайдеров
            for (btRigidBody* rb : rigidBodies)
            {
                if (!rb || rb == playerBody || rb == floorBody) continue;
                ...
                model = glm::mat4(1.0f);
                model = glm::translate(model, btToGlm(tr.getOrigin()));
    ...
    std::cout << "Flux3D Engine shutdown." << std::endl;
    return 0;
}